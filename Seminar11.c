...
Masina popStack(Nod** varfStiva) {
	...
	Nod* temp = *varfStiva;
	Masina tempMasina = temp->info;
	*varfStiva = temp->next;
	free(temp);
	return tempMasina;
}

int isEmptyStack(Nod* varfStiva) {
...
void dezalocareStivaDeMasini(Nod** varfStiva) {
	while (*varfStiva)
	{
		Masina m = popStack(varfStiva);
		free(m.model);
		free(m.numeSofer);
	}
}

int size(Nod* varfStiva) {
	int count = 0;
	while (varfStiva)
	{
		count++;
		varfStiva = varfStiva->next;
	}
	return count;
}
